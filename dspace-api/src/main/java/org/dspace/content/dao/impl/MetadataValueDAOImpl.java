 ...
 * for the MetadataValue object and is autowired by Spring.
...
public class MetadataValueDAOImpl extends AbstractHibernateDAO<MetadataValue> implements MetadataValueDAO {
    ...
    public MetadataValue getMinimum(Context context, int metadataFieldId)
        ...
        if (context.getDbType().equals(DatabaseUtils.DBMS_ORACLE)) {
            queryString = "SELECT m FROM MetadataValue m JOIN FETCH m.metadataField"
                    + " WHERE m.metadataField.id = :metadata_field_id"
                    + " ORDER BY DBMS_LOB.substr(text_value , 4000 , 1)";
        } else {
            queryString = "SELECT m FROM MetadataValue m JOIN FETCH m.metadataField"
                    + " WHERE m.metadataField.id = :metadata_field_id"
                    + " ORDER BY text_value";
